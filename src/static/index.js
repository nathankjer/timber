...
function renderSheetList() {
  ...
  sheets.forEach(s => {
    ...
    li.className = 'list-group-item d-flex justify-content-between align-items-center sheet-item';
...
async function createSheet() {
  const resp = await fetch('/sheet', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'Untitled' }) });
  if (resp.ok) {
...
function solidScreenRect(el) {
  ...
  return { left: c.x - h / 2, right: c.x + h / 2, top: c.y - v / 2, bottom: c.y + v / 2 };
}
  ...
  if (selectedId !== null) {
    ...
    if (el) {
      ...
      if (el.type === 'Member' || el.type === 'Cable') {
        ['E', 'A', 'I'].forEach(p => addNumberInput(form, `${PROP_LABELS[p]} (${p})`, p, el));
      }
      if (el.type === 'Plane') {
        ...
        ['ux', 'uy', 'rz'].forEach(p => {
          ...
          input.addEventListener('change', () => { el[p] = input.checked; saveState(); });
...
function startDrag(ev) {
  ...
  if (el.type === 'Member' || el.type === 'Cable') {
    ...
    const p2 = screenCoords({ x: el.x2 ?? el.x, y: el.y2 ?? el.y, z: el.z2 ?? el.z });
    ...
    const m = 6;
    if (Math.abs(mx - rect.left) < m && my >= rect.top - m && my <= rect.bottom + m) dragMode = 'left';
    else if (Math.abs(mx - rect.right) < m && my >= rect.top - m && my <= rect.bottom + m) dragMode = 'right';
    else if (Math.abs(my - rect.top) < m && mx >= rect.left - m && mx <= rect.right + m) dragMode = 'top';
    else if (Math.abs(my - rect.bottom) < m && mx >= rect.left - m && mx <= rect.right + m) dragMode = 'bottom';
    else dragMode = 'body';
  }
...
function onDrag(ev) {
  ...
  if (el.type === 'Member' || el.type === 'Cable') {
    if (dragMode === 'start') {
      ['x', 'y', 'z'].forEach(k => { el[k] = dragOrig[k] + (delta[k] || 0); });
    } else if (dragMode === 'end') {
      ['x2', 'y2', 'z2'].forEach(k => { const a = k[0]; el[k] = dragOrig[k] + (delta[a] || 0); });
    } else {
...
function buildModel() {
  ...
  const members = elements.filter(e => e.type === 'Member' || e.type === 'Cable').map(e => ({
    start: idx(e.x, e.y),
  ...
  const loads = elements.filter(e => e.type === 'Load').map(e => {
    const i = idx(e.x, e.y);
  ...
  const supports = elements.filter(e => e.type === 'Support').map(e => ({
    joint: idx(e.x, e.y),
    ux: e.ux !== false,
    uy: e.uy !== false,
    rz: e.rz !== false,
...
async function solveModel() {
  ...
  for (const [k, v] of Object.entries(data.reactions || {})) {
    lines.push(`Joint ${k}: fx=${(+v[0]).toExponential(3)} N, fy=${(+v[1]).toExponential(3)} N, mz=${(+v[2]).toExponential(3)} N·m`);
  }
